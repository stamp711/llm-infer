...
class TensorStaticMutView;

// Traits specialization for TensorStaticMutView
template <typename T, std::size_t... Dims>
struct TensorInnerTypes<TensorStaticMutView<T, Dims...>> {
    using value_type = T;
...
// Mutable view of fully static tensor - doesn't own data
template <typename T, std::size_t... Dims>
class TensorStaticMutView : public MutableTensorBase<TensorStaticMutView<T, Dims...>> {
   public:
    using base_type = MutableTensorBase<TensorStaticMutView<T, Dims...>>;
    using inner_types = TensorInnerTypes<TensorStaticMutView<T, Dims...>>;
   ...
   private:
    T* data_ptr_ = nullptr;  // Non-owning pointer to mutable data
    static constexpr shape_type shape_ = {Dims...};

   public:
    TensorStaticMutView() = default;

    // View from raw pointer - verifies size matches
    explicit TensorStaticMutView(T* data) : data_ptr_(data) {}

    // CRTP implementations
    [[nodiscard]] T* data_impl() noexcept { return data_ptr_; }
