...
struct LlamaConfig {
    ...
    [[nodiscard]] bool uses_gqa() const noexcept {
        return attention_head_count_kv.has_value() && *attention_head_count_kv != attention_head_count;
    }

    [[nodiscard]] bool is_moe() const noexcept { return expert_count.has_value() && *expert_count > 0; }

    [[nodiscard]] std::uint64_t head_dim() const noexcept { return embedding_length / attention_head_count; }
